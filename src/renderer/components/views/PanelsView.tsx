...
type CardAction = 'connecting' | 'disconnecting' | 'syncing' | 'deleting' | 'editing' | null;
...
const PanelsView = () => {
  ...
  const handleDismissCardError = useCallback(async (panelId: string) => {
    // Clear transient per-card error immediately.
    setCardError(panelId, null);
    // If error came from persisted panel.error, clear it too so it stays dismissed.
    setPanels(prev => prev.map(p => (p.id === panelId ? { ...p, error: undefined } : p)));
    try {
      await updatePanel(panelId, { error: undefined });
    } catch {
      // Keep UI dismissal even if persistence fails; next loadData can rehydrate server truth.
    }
  }, [setCardError]);
  ...
  const handleSaveEdit = async (data: { name: string; url: string; username: string; password?: string }) => {
    ...
    setCardAction(editingPanel.id, 'editing');
    try {
                ...
                {cardError && (
                  <div className="pcard-error-inline">
                    ...
                    <button
                      ...
                      onClick={() => { void handleDismissCardError(panel.id); }}
                      title="Dismiss"
                    >
                      <i className="fas fa-times"></i>
                    </button>
